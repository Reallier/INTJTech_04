...
export default defineEventHandler(async (event) => {
    ...
    if (!user) {
        ...
        if (recentRegistrations >= 3) {
            console.log('[Auth Callback] IP 注册频率限制:', clientIp, recentRegistrations);
            throw createError({ statusCode: 429, statusMessage: '注册过于频繁，请稍后再试' });
        }
        ...
        console.log('[Auth Callback] 赠送新用户额度:', NEW_USER_BONUS, 'IP:', clientIp);
    } else {
